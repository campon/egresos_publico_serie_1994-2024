---
title: "Egresos del Sector Público"
...
Los **Egresos Primarios del Sector Público no Monetario**, son una fuente crucial de información proporcionada por el [Ministerio de Economía y Finanzas](https://www.gub.uy/ministerio-economia-finanzas/datos-y-estadisticas/estadisticas/informacion-resultados-del-sector-publico). Esta serie mensual, ofrece una panorámica detallada de los desembolsos financieros del sector público, a lo largo del tiempo.
...
Se explorará la serie temporal con el
objetivo de identificar un modelo adecuado que permita reproducirl, facilitando así la capacidad predictiva sobre los egresos del
sector público en Uruguay.

## Serie temporal: Gasto Público

```{r plot_series_inicial, echo=FALSE, out.width="85%", fig.align ="center", fig.pos="H", fig.cap = "\\label{gasto_publico} Evolución del Gasto Público mensual en millones de pesos corrientes entre enero 1999 y abril 2024.", results='asis'}
...
```{r month_plot, fig.align = 'center', echo=FALSE, out.width="85%", fig.cap = "\\label{month_plot} Comportamiento mensual del Gasto Público entre 1999 y 2024.", fig.pos = 'H'}
...
```{r season_plot, echo = F, fig.align = 'center',out.width="85%", fig.cap = "\\label{seasonal_plot} Comportamiento por año del Gasto Público entre 1999 y 2024.", fig.pos = 'H'}
...
El desarrollo esta basado en la metodología de Box y Jenkins, en la construcción de modelos ARIMA para el análisis de series de tiempo. Podemos dividir esta metodología en cuatro grandes etapas:
...
La identificación del modelo, esta basada en las funciones de autocorrelación y autocorrelación parcial y a partir de ello, podremos obtener diferentes modelos candidatos. Tras la estimación, un diagnostico con determinadas etapas es realizado; y por último, aplicaremos predicciones sobre los mismos y se evaluará su desempeño.

## Logaritmo de la serie

```{r log_plot, fig.align = 'center', fig.align = 'center', out.width="85%" , fig.cap = "\\label{log_plot} Evolución del logaritmo del Gasto Público mensual entre 1999 y 2024.", fig.pos = 'H', warning = FALSE, message = FALSE}
...
```{r, fig.align = 'center', fig.align = 'center', out.width="85%", fig.cap = "\\label{fac_facp} Funciones de Autocorrelación y Autocorrelación Parcial estimadas del logaritmo del Gasto Público.", fig.pos = 'H'}
...
Este test contrasta en su hipótesis nula que el proceso contiene una raíz
unitaria y por lo tanto que es no estacionario, mientras que
la alternativa es que no presenta raíz unitaria y por lo tanto, se
cumple el supuesto de estacionariedad. 

La siguiente ecuación es la regresión auxiliar con tendencia determínistica para la solución paramétrica de DF (1979):
...
## Funciones de Autocorrelación serie diferenciada

```{r, fig.align = 'center', out.width="85%" , fig.cap = "\\label{fac_facp_diff} Funciones de Autocorrelación y Autocorrelación Parcial estimadas para la primera diferencia regular del logaritmo del Gasto Público.", fig.pos = 'H'}
...
```{r, fig.align = 'center', out.width="70%", fig.cap = "\\label{fac_facp_estac} Funciones de Autocorrelación y Autocorrelación Parcial estimadas para la primera diferencia estacional del logaritmo del Gasto Público (1999-2024).", fig.pos = 'H'}
...
Se identifican dos modelos posibles, **SARIMA(0,0,1)(0,1,1)** y **SARIMA(0,0,1)(1,1,0)**.
...
```{r, fig.align = 'center', out.width="70%", fig.cap = "\\label{dd_fac_facp} Funciones de Autocorrelación y Autocorrelación Parcial estimadas para la primera diferencia regular de la primera diferencia estacional del logaritmo del Gasto Público.", fig.pos = 'H'}
...
## Diagnóstico: Residuos y residuos estandarizados

```{r residos, include=FALSE}
...
## Diagnóstico:Función de autocorrelación y autocorrelación parcial de los residuos
...
## Diagnóstico: Tests de Ljung-Box y Shapiro-Wilks
...
Veánse los resultados de la incorrelación y normalidad de los residuos en el cuadro \ref{tab:diagnostico}.
...
residuos4_acf <- ggAcf(residuos4, lag.max = 36, type = "correlation") +
  labs(x = "Rezago",
       y = "Autocorrelación",
       title = "")

residuos4_pacf <- ggAcf(residuos4, lag.max = 36, type = "partial") +
  labs(x = "Rezago",
       y = "Autocorrelación parcial",
       title = "")
...
```{r, fig.align = 'center', out.width="85%", fig.cap = "\\label{residuos4} Residuos normales y estandarizados de un modelo SARIMA(0,1,1)(0,1,1) intervenido para el logaritmo del Gasto Público.", fig.pos = 'H'}
grid.arrange(grafico_residuos4, grafico_residuos_est4)
```



## Diagnostico: Función de autocorrelación y autocorrelación parcial de los residuos

```{r, fig.align = 'center', out.width="75%", fig.cap = "\\label{facyp_r4} Funciones de Autocorrelación y Autocorrelación Parcial estimadas de los residuos de un modelo SARIMA(0,1,1)(0,1,1) intervenido para el logaritmo del Gasto Público.", fig.pos = 'H'}
grid.arrange(residuos4_acf, residuos4_pacf)
```

## Diagnostico: Tests de Ljung-Box y Shapiro-Wilks
...
## Diagnóstico: Histograma de los residuos

```{r, fig.align = 'center', out.width="80%", fig.cap = "\\label{norms} Histograma de los residuos de los modelos reestimados SARIMA intervenido spara el logaritmo del Gasto Público. La línea roja corresponde a una densidad normal con media y desvío muestrales igual al de los residuos.", fig.pos = 'H', warning = FALSE, message = FALSE}
...
Las métricas de error que se utilizarán para comparar los modelos son:
  ...
  - Error absoluto medio (MAE): mide la magnitud absoluta promedio del desvío de la predicción del valor real.

- Error porcentual absoluto medio (MAPE): mide la magnitud promedio del desvío de la predicción del valor real en términos porcentuales.

- Error medio absoluto escalado (MASE): mide la magnitud promedio del desvío de la predicción del valor real, escalada por el MAE del conjunto de entrenamiento.

Un valor más cercano a cero indica una mejor predicción para las tres métricas.

## Predicción de los modelos 1 y 2
...
## Predicción de los modelos 3 y 4
